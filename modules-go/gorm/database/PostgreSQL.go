...
type ContentCensorModel struct {
	...
	Level         int       `json:"level" gorm:"column:level;size:2;defalut:-1;comment:敏感词等级"`
	Rule          *string   `json:"rule" gorm:"column:rule;comment:敏感词规则"` // 使用*号可以 默认是空字符串
	Risky         *string   `json:"risky" gorm:"column:risky;comment:敏感词"` // 使用*号可以 默认是空字符串
